        ...
        $.ajax({
            ...
            success: function (response) {
                if (response != null) {
                    ...
                    $(".extra-address-select").html('<option value="-1">Dirección temporal</option><option selected value="0">Dirección predeterminada</option>' + html);
                    if (route == "/") {
    ...
    function closeSale() {
        ...
        if (chatActive != "0" && delivery != "0" && delivery != "-1") {
            if (address == "-1") {
                const customAddress = ($("address-custom-temp").val()).trim();
                if (customAddress != "") {
    ...
    $(document).ready(function () {
        $("#select-delivery").change(function () {
            var value = $(this).val();
            if (value == "0") {
                $("#label-delivery-note").text("Nota para el cliente.");
                $('#msg-add-delivery').prop('placeholder', 'Nota para el cliente...');
                $("#select-address").prop('disabled', true);
            } else {
                $("#label-delivery-note").text("Nota para el repartidor.");
                $('#msg-add-delivery').prop('placeholder', 'Nota para el repartidor...');
